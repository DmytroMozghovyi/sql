...
SELECT v.vendor_id, market_date
FROM vendor v
INNER JOIN vendor_booth_assignments vba
	ON v.vendor_id = vba.vendor_id
ORDER BY v.vendor_id, market_date
...
SELECT vendor_id, count (vendor_id) number_of_booth_rents
...
SELECT customer_id,
	   strftime('%Y', market_date) as year,
	   strftime('%m', market_date) as month,
	   sum(quantity*cost_to_customer_per_qty) as expenses
FROM customer_purchases
